...
DEFAULT_WIDTH = 600
DEFAULT_HEIGHT = 560
PROGRESS_AREA_HEIGHT = 70  # Extra height for progress bar area
...
def hide_progress_indicators():
    ...
    app.geometry(f"{app.winfo_width()}x{DEFAULT_HEIGHT}")
...
def run_with_progress(task_name, actions):
    ...
    def task_done():
        ...
        for btn in (copy_btn, import_btn, export_btn, open_btn):
...
def copy_folder_contents():
    try:
        ...
        if actions:
            ...
            show_custom_message("Success", "Project created successfully!")
    except Exception as e:
...
ctk.CTkLabel(frame, text="v1.0.2", font=("Segoe UI", 10), text_color="gray").pack(pady=(0, 10))

hide_progress_indicators()

# ---------- Check for file opened with app ----------
check_startup_file()
